...
def get_api_info():
    ...
    if api_choice == "Gemini":
        ...
        return api_choice, "gpt-4o-mini", None, None, max_tokens
...
def process_image_openai(image_file, prompt, max_tokens):
    try:
...
def analyze_image(section_id, image_file, api_choice, model, temperature, top_p, max_tokens):
    if image_file is not None:
        ...
        prompt = st.text_input(f"Enter a prompt for Image {section_id}", key=f"prompt_{section_id}")
        
        if st.button(f"Analyze Image {section_id}"):
            if prompt:
                with st.spinner("Analyzing image..."):
                    logger.info(f"Starting analysis for image {section_id} using {api_choice}")
                    if api_choice == "Gemini":
                        gemini_model = genai.GenerativeModel(model_name=model)
                        analysis = process_image_gemini(image_file, prompt, gemini_model, temperature, top_p, max_tokens)
                    else:
                        analysis = process_image_openai(image_file, prompt, max_tokens)
                    ...
                    if analysis:
                        ...
                        logger.info(f"Successfully generated analysis for image {section_id}")
                    else:
