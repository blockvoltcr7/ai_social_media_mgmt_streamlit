...
def analyze_image(section_id, image_file, model_instance):
    if image_file is not None:
        ...
        if analyze_button and prompt:
            with st.spinner("Analyzing image..."):
                logger.info(f"Starting analysis for image {section_id}.")
                analysis = model_instance.process_image(image_file, prompt)
...
def main():
    ...
    model_instance = ModelFactory.get_model(api_choice, model_name=model, temperature=temperature, top_p=top_p, max_tokens=max_tokens)
    ...
    if uploaded_files:
        ...
        for i, file in enumerate(uploaded_files):
            with cols[i]:
                analyze_image(i+1, file, model_instance)
